document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getOrCreateErrorEl(field) {
      const fieldWrapper = field.closest('.form-group, .field-wrapper, div') || field.parentNode;
      ...
      if (!errorEl) {
        ...
        errorEl.style.cssText = 'color:#e53e3e;font-size:0.85em;display:block;margin-top:4px;';
    ...
    function validateField(field) {
      ...
      if (isRequired && !validators.required(value)) {
        setFieldError(field, errorMessages.required);
        return false;
      }
      ...
      if (fieldType === 'tel' && !validators.phone(value)) {
        setFieldError(field, errorMessages.phone);
        return false;
      }

      if (minLength && !validators.minLength(value, minLength)) {
        setFieldError(field, errorMessages.minLength.replace('{min}', minLength));
        return false;
      }
    ...
    contactForm.addEventListener('input', function (e) {
      const field = e.target;
      if (field.matches('input, textarea, select') && field.classList.contains('field-invalid')) {
        validateField(field);
      }
    });
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (submitBtn) {
        ...
        submitBtn.textContent = submitBtn.dataset.loadingText || 'جاري الإرسال...';
        setTimeout(function () {
  ...
  animationStyles.textContent = [
    '.fade-in-element{opacity:0;transform:translateY(24px);transition:opacity 0.6s ease,transform 0.6s ease;}',
    '.fade-in-element.fade-in-visible{opacity:1;transform:translateY(0);}'
  ].join('');
  ...
  if (elementsToAnimate.length > 0 && 'IntersectionObserver' in window) {
    ...
    elementsToAnimate.forEach(function (el, index) {
      ...
      if (!el.dataset.delay && !el.dataset.animateDelay) {
        const siblings = el.parentNode ? el.parentNode.querySelectorAll(animationSelectors) : [];
        let siblingIndex = 0;
        siblings.forEach(function (sib, i) { if (sib === el) siblingIndex = i; });
        if (siblings.length > 1 && siblingIndex < 6) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "דן המחלץ משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '6cfbc52a-746d-4cf0-a763-1ff92aa6e4ba',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '6cfbc52a-746d-4cf0-a763-1ff92aa6e4ba';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '6cfbc52a-746d-4cf0-a763-1ff92aa6e4ba';
